...
const { ajaxurl, nonce, selfVersion, selfRepo } = wpteDbg;
...
export class MarketplaceTab {
	...
	_makeCard( plugin ) {
		...
		if ( plugin.github_repo ) {
			header.appendChild( this._makeRepoLink( plugin.github_repo ) );
		}
	...
	/** Build the GitHub icon link shown in a card header for a given repo slug. */
	_makeRepoLink( githubRepo ) {
		...
		repoLink.href      = 'https://github.com/' + githubRepo;
		repoLink.target    = '_blank';
		repoLink.rel       = 'noopener noreferrer';
		repoLink.title     = githubRepo;
	...
	_makeSelfCard() {
		...
		if ( selfRepo ) {
			header.appendChild( this._makeRepoLink( selfRepo ) );
		}
